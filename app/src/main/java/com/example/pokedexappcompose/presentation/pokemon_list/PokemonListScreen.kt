    ...
    Box(modifier = Modifier.fillMaxSize()){
        if (state.pokemonList.isNotEmpty()){
            LazyColumn(contentPadding = PaddingValues(16.dp)){
                ...
                //burası launchefect ile yapılabilir.
                items(itemCount){
                    if (it >= itemCount-1 && !state.endReached && !state.isLoading){
                        viewModel.loadPokemonPaging()
                    }
                    PokemonRow(rowIndex = it , entries = state.pokemonList , navController = navController)
                }
            }
        }
